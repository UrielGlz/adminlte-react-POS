              ...
              {loading ? (
                ...
                <div className="timeline">
                  {history.map((item, idx) => {
                    const fromLabel = [item.from_account_number, item.from_account_name].filter(Boolean).join(' / ')
                    const toLabel = [item.to_account_number, item.to_account_name].filter(Boolean).join(' / ')
                        ...
                        <div className="card flex-grow-1 shadow-sm">
                          <div className="card-body py-2 px-3">
                            ...
                            <p className="mb-1 small">
                              ...
                              {item.is_initial_assignment ? 'assigned the ticket to' : 'changed customer from'}
                              {!item.is_initial_assignment && (
                                <>
                                  {' '}<code>{fromLabel || 'No previous customer'}</code>{' '}to
                                </>
                              )}
                              {' '}<code>{toLabel || 'Unknown customer'}</code>
                            </p>
